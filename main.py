...
class Character:
    ...
    def __init__(self: "Character", name: str) -> None:
        self.name = name
    """У каждого персонажа есть имя, можно
    доверить его родительскому классу."""

    def attack(self: "Character") -> str:
        """
        функции отвечает за генерацию очков урона,
        нанесённого противнику.
        """
        value_attack = DEFAULT_ATTACK + randint(*self.RANGE_VALUE_ATTACK)
        return (f'{self.name} нанёс противнику урон, равный {value_attack}')

    def defence(self: "Character") -> str:
        """Функция возвращает урон при защите персонажа."""
        value_defence = DEFAULT_DEFENCE + randint(*self.RANGE_VALUE_DEFENCE)
        return (f'{self.name} блокировал {value_defence} ед. урона.')

    def special(self: "Character") -> str:
